...
function normalizeQuote({ marketId, yesBid, yesAsk, timestamp, mode, simulated, asset, price, windowEnd, expectedExpiration }) {
  ...
  if (windowEnd != null) quote.windowEnd = Number(windowEnd);
  if (expectedExpiration != null) quote.expectedExpiration = Number(expectedExpiration);
  return quote;
}
