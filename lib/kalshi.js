...
/* ------------------------------------------------------------------ *
 * KalshiFeed — live Kalshi KXBTC15M binary BTC Up/Down feed.
 *
 * Polls the public Kalshi markets endpoint for the "Bitcoin price up
 * down" series (15-minute rolling contracts) and emits quotes shaped
 * exactly like every other adapter in this repo:
 *
 *   { marketId, yesBid, yesAsk, noBid, noAsk, mid, price, timestamp,
 *     mode, simulated, asset, windowEnd, strike, settlementSeconds }
 *
 * The feed selects the ACTIVE contract for the current 15-minute window
 * (openTime <= now < closeTime) and derives the YES/NO book from its
 * public bid/ask in dollars. Reads are unauthenticated and verified
 * against api.elections.kalshi.com/trade-api/v2. No simulated data is
 * ever emitted: when no window is live the feed stays idle and logs.
 *
 * The momentum price carried on each quote is the live BTC spot from
 * lib/btc-price.js (public Coinbase ticker) — used by the engine's
 * momentum signal and the dashboard BTC row. Kalshi settlement itself
 * uses CF Benchmarks BRTI per the contract rules; the engine derives the
 * natural paper result from the BTC spot at the window close.
 *
 * Emits:
 *   tick   { quote }
 *   market { marketId, closeTime, ... }  on 15-min window rollover
 * ------------------------------------------------------------------ */
...
/**
 * Returns the market close timestamp (ms) derived from a contract ticker
 * like KXBTC15M-26AUG202300-00, or null if it cannot be parsed.
 */
function parseTickerCloseTime(ticker, reference = Date.now()) {
  ...
  if (Math.abs(close - reference) > 3 * DAY_MS || utcDay(close) !== utcDay(reference)) {
    // Snap to the nearest occurrence of this HH:MM around `reference`.
...
class KalshiFeed extends EventEmitter {
  constructor(config) {
    ...
    this.pollMs = config.KALSHI_POLL_MS || 5000;

    this.active = null; // { market, openTime, closeTime, expectedExpiration }
  ...
  start() {
    ...
    const poll = () => this._poll().catch(() => this._safeContinue());
    poll();
  ...
  /**
   * Natural settlement value for a held direction at window close.
   * YES (UP) wins when the closing BTC spot >= the window-open strike;
   * NO (DOWN) wins when it is below. Paper-fill settles at 1.0 / 0.0.
   */
  ...
  async _poll() {
    ...
    const now = Date.now();
    let active = null;
    // Prefer a contract whose window is currently open.
    ...
    // Fallback: no window "open" by the ticker convention, but Kalshi lists
    // the next-day schedule early. Take the contract that CLOSES next.
    if (!active) {
      let best = null;
      for (const m of markets) {
    ...
    // Keep momentum strike: at window rollover, snapshot spot -> window-open strike.
  ...
  _safeContinue() {
    // Backoff already handled by _backoff(); keep the interval running.
  }
