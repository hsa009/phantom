...
class PaperEngine extends EventEmitter {
  ...
  _onTick(quote) {
    ...
    this._buffer.push({ mid: quote.mid, ts: now });
  ...
  _decideDirection(quote) {
    ...
    const shortMs = Math.max(500, Math.round(lookback / 3));

    const recent = this._buffer.filter((s) => now - s.ts <= shortMs);
    const long = this._buffer.filter((s) => now - s.ts <= lookback);

    if (long.length < 2 || recent.length === 0) {
      return {
        canOpen: false,
        reason: 'insufficient_data',
        detail: `buffer span=${(now - (this._buffer[0]?.ts || now))}ms lookback=${lookback}ms`,
      };
    }

    const avg = (arr, key) => arr.reduce((s, x) => s + x[key], 0) / arr.length;
    const shortAvg = avg(recent, 'mid');
    const longAvg = avg(long, 'mid');
    ...
    return {
      ...
      detail: `momentum=${momentum.toFixed(6)}`,
    };
  }
