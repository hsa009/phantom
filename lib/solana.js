 ...
 *   - on('market', { marketId })
 *
 * Quote shape (the single normalization every adapter produces):
 *   { marketId, yesBid, yesAsk, noBid, noAsk, mid, timestamp,
 *     mode, simulated }
 * with  noBid = 1 - yesAsk  and  noAsk = 1 - yesBid.
 *
 * SIM  -> SimFeed     in-process random-walk order book for verifying the
 *                     Supabase / Telegram / WebSocket plumbing.
 * REAL -> DriftFeed   live Drift B.E.T (Prediction) market oracle feed over
 *                     a mainnet Helius RPC WebSocket (@drift-labs/sdk).
 *                     Never emits simulated data: on failure it logs and
 *                     retries with backoff while staying idle.
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ *
 * SimFeed — realistic BTC spot-price simulator.
 *
 * Simulates the TARGET_ASSET spot price with a geometric random walk
 * tuned to ~0.5-1% total volatility per 5-minute window, plus occasional
 * jump shocks so the engine's momentum signal has real structure. The
 * YES (up) probability is derived from where BTC sits relative to the
 * window-open strike, and a bid/ask book is built around that odds.
 * ------------------------------------------------------------------ */
class SimFeed extends EventEmitter {
  ...
  _step() {
    ...
    // Occasionally shock the BTC price so momentum has structure to read.
    ...
    // Geometric random walk (per-tick vol ~ total-5min-vol / sqrt(ticks)).
    this._price *=
      1 + c.SIMS_BTC_DRIFT + c.SIMS_BTC_SIGMA * gaussian();
    this._price = Math.max(c.SIMS_BTC_START_PRICE * 0.5, this._price);

    // 5-minute window rollover -> new strike and marketId.
    ...
    // YES (up) probability from price position vs the window-open strike.
...
function getMarketFeed(config) {
  const feed =
    config.MARKET_MODE === 'real' ? new DriftFeed(config) : new SimFeed(config);
